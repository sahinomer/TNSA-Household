...
```{r}
household <- read.table(file="household.csv", sep=";", header = TRUE, 
...
household <- separate(household, region_combined, 
                      c("cardinal_direction", "region", "settlement"), sep = '-')
...
household$man_member_rate <- (household$household_member - household$woman_member)
household$man_member_rate <- household$man_member_rate / household$household_member  
...
```{r}
ggplot(household, aes(x = wealth_index)) + geom_bar(fill="steelblue") + 
...
ggplot(df, aes(x = Model, weight=Accuracy)) + geom_bar(fill="steelblue") + 
...
# References
   ...
   - TNSA Dataset http://www.hips.hacettepe.edu.tr/eng/population_survey.shtml
   - Lecture Notes
   - GGPlot https://ggplot2.tidyverse.org/
   - R Documentations https://www.rdocumentation.org/
   - H2O Neural Network http://docs.h2o.ai/h2o/latest-stable/h2o-docs/data-science/deep-learning.html
