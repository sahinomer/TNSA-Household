...
SVM crates decision boundry between classes. Cost and gamma parameters are evaluated by performance estimation. The test model is trained with the optimum paramater set for the classification.
...
### Adabag
Bootstrap Aggregation (Bagging) is an ensemble method. The number of iterations for boosting run and maxdepth is evaluated by using performance estimator.
...
### Random Forest
Random Forest is combination of the multiple decision trees to prevent overfitting that comes with decision trees. The number of trees is evaluated by using performance estimator.
