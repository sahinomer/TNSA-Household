...
At the end of data preparation, some data analyzing by visualizing is done.


The relation between house ownership and wealth index:
```{r echo=FALSE}
...
The relation between car ownership and wealth index:
```{r echo=FALSE}
tbl <- table(household$wealth_index, household$car)
tbl <- tbl / rowSums(tbl)
conf_matrix <- as.data.frame(tbl)
ggplot(data =  conf_matrix, mapping = aes(x = Var1, y = Var2)) +
   xlab("Wealth Index") + ylab("Car Ownership") + 
   geom_tile(aes(fill = Freq)) +
   geom_label(aes(label = sprintf("%0.4f", Freq)), vjust = 0.5) +
   scale_fill_gradient(low = "white",
                       high = "blue")
...
```{r}
decision_tree <- rpartXse(wealth_index ~ ., train[,c("wealth_index", "internet")], se = 1)
